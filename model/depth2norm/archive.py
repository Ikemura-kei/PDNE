...
def depth2norm(depth, camera_matrix, neighborhood_size=3, computation_mode="row", step_size=50):
    '''
    depth: a torch tensor of depth map, has shape (B, 1, H, W), values should be in the unit of m
    ...
    '''
    B, _, H_orig, W_orig = depth.size()
    
    depth_squeezed = torch.squeeze(depth, dim=1) * 1000.0 # m to mm
    ...
    depth_squeezed = torch.nn.functional.pad(depth_squeezed, pad=total_padding)
    
    depth_patches = depth_squeezed.unfold(1, patch_size, 1).unfold(2, patch_size, 1)
    ...
    xx_over_z = torch.from_numpy(xx_over_z).to(depth_squeezed.get_device())
    yy_over_z = torch.from_numpy(yy_over_z).to(depth_squeezed.get_device())
    ...
    ##########################################################################################################################
        ...
        tmp_det = torch.det(tmp.cpu())
        # tmp_det = None
        # for k in range(B):
        #     tmp_det_ = torch.det(tmp[k]) # (B, H, W), compute in cpu
        #     if tmp_det is None:
        #         tmp_det = tmp_det_[None,...]
        #     else:
        #         tmp_det = torch.cat([tmp_det, tmp_det_[None,...]])
        ...
        tmp_inversible = torch.where(tmp_det >= 1e-6, tmp, torch.eye(3).to(tmp.get_device()).double())
        norm_unnormalized = torch.matmul(torch.matmul(torch.inverse(tmp_inversible), A_T), torch.ones(patch_size**2).to(A_T.get_device()).double())
    ...
    norm_normalized = torch.nn.functional.normalize(norm_unnormalized, dim=3).float()
