...
def depth2norm(depth, camera_matrix, neighborhood_size=3, computation_mode="row", step_size=50):
    ...
    # print("max depth is: ", torch.max(depth))
    depth_sq = torch.squeeze(depth, dim=1) * 1000.0
    ...
    depth_sq = torch.nn.functional.pad(depth_sq, pad=total_padding)
    
    depth_patches = depth_sq.unfold(1, patch_size, 1).unfold(2, patch_size, 1)
    ...
    ##########################################################################################################################
        ...
        # print("tmp_det", torch.mean(tmp_det))
        # print(tmp_det.size())
        ...
        # print("tmp_inversible", torch.mean(tmp_inversible))
        ...
        # print("norm_unnormalized", torch.mean(norm_unnormalized))
    ...
    # print("norm_normalized", torch.mean(norm_normalized))
