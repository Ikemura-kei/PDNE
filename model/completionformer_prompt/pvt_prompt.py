...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 sr_ratios=[8, 4, 2, 1], num_stages=4, pretrained=None, use_prompt=False):
        super().__init__()
        self.depths = depths
    ...
    def forward_features(self, x):
        ...
        print("input is NaN? {}".format(torch.any(torch.isnan(x))))
        x = embed_l1(x)
        print("output 1 is NaN? {}".format(torch.any(torch.isnan(x))))
        outs.append(x)
        x = embed_l2(x)
        print("output 2 is NaN? {}".format(torch.any(torch.isnan(x))))
...
class PVTPrompt(PyramidVisionTransformer):
    def __init__(self, in_chans, patch_size=4, **kwargs):
        super(PVTPrompt, self).__init__(
            ...
            sr_ratios=[8, 4, 2, 1], drop_rate=0.0, drop_path_rate=0.1, pretrained=kwargs['pretrained'], use_prompt=True)
