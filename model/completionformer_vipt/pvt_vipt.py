...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        if use_prompt:
            self.prompt_modifier0 = nn.Sequential(nn.Conv2d(48, chs[0], kernel_size=3, stride=2, padding=1), \
                                                nn.ReLU())
        ...
        for i in range(num_stages):
            if use_prompt:
                ...
                prompt_modifier = nn.Sequential(nn.Conv2d(chs[i], chs[i], kernel_size=3, stride=2, padding=(1 if i != (num_stages-1) else 0)), \
                                                nn.ReLU())
...
class PVTViPT(PyramidVisionTransformer):
    def __init__(self, in_chans, patch_size=4, foundation=None, **kwargs):
        super(PVTViPT, self).__init__(
