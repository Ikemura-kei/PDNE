...
from .backbone_vipt import BackboneViPT

class CompletionFormerViPT(nn.Module):
    def __init__(self, args):
        super(CompletionFormerViPT, self).__init__()
        ...
        self.backbone = BackboneViPT(args, self.foundation.backbone, mode='rgbd')

        if self.prop_time > 0:
            self.prop_layer = self.foundation.prop_layer
            # for params in self.prop_layer.parameters():
