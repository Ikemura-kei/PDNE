...
class Backbone(nn.Module):
    def __init__(self, args, mode='rgbd'):
        ...
        if mode == 'rgbd':
            ...
            if self.args.pol_rep == 'grayscale-4':
                self.conv1_pol = conv_bn_relu(4, 32, kernel=3, stride=1, padding=1,
                                          bn=False)
            elif self.args.pol_rep == 'rgb-12':
                self.conv1_pol = conv_bn_relu(12, 32, kernel=3, stride=1, padding=1,
                                          bn=False)
            elif self.args.pol_rep == 'leichenyang-7':
                self.conv1_pol = conv_bn_relu(7, 32, kernel=3, stride=1, padding=1,
                                          bn=False)

            self.conv1 = conv_bn_relu(96, 64, kernel=3, stride=1, padding=1,
                                      bn=False)
        elif mode == 'rgb':
    ...
    def forward(self, rgb=None, depth=None, pol=None):
        ...
        if self.mode == 'rgbd':
            ...
            fe1_pol = self.conv1_pol(pol)
            fe1 = torch.cat((fe1_rgb, fe1_dep, fe1_pol), dim=1)
            fe1 = self.conv1(fe1)
        elif self.mode == 'rgb':
            fe1 = self.conv(rgb)
        elif self.mode == 'd':
