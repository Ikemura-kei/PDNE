...
class BackbonePromptFinetuneV2(nn.Module):
    def __init__(self, args, foundation, mode='rgbd'):
        super(BackbonePromptFinetuneV2, self).__init__()
        self.args = args
        self.mode = mode
        ...
        chs = [256, 128, 64, 64, 64]
        heights = [12, 26, 52, 104, 208]
        widths = [16, 34, 68, 136, 272]
        
        self.prompt_fd6 = nn.Parameter(torch.zeros([1, chs[0], heights[0], widths[0]]))
        nn.init.uniform_(self.prompt_fd6)
        
        self.prompt_fd5 = nn.Parameter(torch.zeros([1, chs[1], heights[1], widths[1]]))
        nn.init.uniform_(self.prompt_fd5)

        self.prompt_fd4 = nn.Parameter(torch.zeros([1, chs[2], heights[2], widths[2]]))
        nn.init.uniform_(self.prompt_fd4)

        self.prompt_fd3 = nn.Parameter(torch.zeros([1, chs[3], heights[3], widths[3]]))
        nn.init.uniform_(self.prompt_fd3)

        self.prompt_fd2 = nn.Parameter(torch.zeros([1, chs[4], heights[4], widths[4]]))
        nn.init.uniform_(self.prompt_fd2)
    ...
    def forward(self, rgb=None, depth=None, pol=None):
        ...
        # Shared Decoding
        fd6 = self.dec6(fe7)
        fd6 = fd6 + self.prompt_fd6.expand(B, -1, -1, -1)
        # print("fd6 shape {}".format(fd6.shape))
        
        fd5 = self.dec5(self._concat(fd6, fe6))
        fd5 = fd5 + self.prompt_fd5.expand(B, -1, -1, -1)
        # print("fd5 shape {}".format(fd5.shape))
        
        fd4 = self.dec4(self._concat(fd5, fe5))
        fd4 = fd4 + self.prompt_fd4.expand(B, -1, -1, -1)
        # print("fd4 shape {}".format(fd4.shape))
        
        fd3 = self.dec3(self._concat(fd4, fe4))
        fd3 = fd3 + self.prompt_fd3.expand(B, -1, -1, -1)
        # print("fd3 shape {}".format(fd3.shape))
        
        fd2 = self.dec2(self._concat(fd3, fe3))
        fd2 = fd2 + self.prompt_fd2.expand(B, -1, -1, -1)
