...
class CompletionFormerFinetune(nn.Module):
    def __init__(self, args):
        super(CompletionFormerFinetune, self).__init__()
    ...
    def forward(self, sample, norm_features):
        rgb = sample['rgb']
        dep = sample['dep']
        pol = sample['pol']

        pred_init, guide, confidence = self.backbone(rgb, dep, pol, norm_features)
